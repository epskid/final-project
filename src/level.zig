...
pub fn load(path: [:0]const u8, game: *Game) !Self {
    const layout_path = try std.fmt.allocPrintSentinel(game.global_allocator, "{s}/layout.txt", .{path}, 0);
    defer game.global_allocator.free(layout_path);

    const layout = rl.loadFileText(layout_path);
    defer rl.unloadFileText(layout);
...
pub fn advance(self: *Self, game: *Game) bool {
    // try to advance the player to another map, returning its success in doing so
